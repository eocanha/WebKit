...
void AppendPipeline::appsinkNewSample(const Track& track, GRefPtr<GstSample>&& sample)
{
    ...
    if (segment && (segment->time || segment->start)) {
    ...
    } else if (!webkitGstCheckVersion(1, 16, 0) && !GST_BUFFER_DTS(buffer) && GST_BUFFER_PTS(buffer) > 0 && GST_BUFFER_PTS(buffer) <= 100'000'000) {
        ...
        GST_DEBUG_OBJECT(pipeline(), "Extending first sample of track '%" PRIu64 "' to make it start at PTS=0 %" GST_PTR_FORMAT, track.trackId, buffer);
