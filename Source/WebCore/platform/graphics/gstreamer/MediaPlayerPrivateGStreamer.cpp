...
void MediaPlayerPrivateGStreamer::createGSTPlayBin(const URL& url)
{
    ...
    g_signal_connect_swapped(bus.get(), "sync-message::state-changed", G_CALLBACK(+[](MediaPlayerPrivateGStreamer* player, GstMessage* message) {
        ...
        installOrUninstallProbeIfNeeded(player, "webkit-gl-video-appsink", "sink", GST_STATE_READY, debugProbeId++, message);
        installOrUninstallProbeIfNeeded(player, "avdec*", "sink", GST_STATE_READY, debugProbeId++, message);
        installOrUninstallProbeIfNeeded(player, "avdec*", "src", GST_STATE_READY, debugProbeId++, message);
